---
name: apple-silicon-llm-speed-lab
description: "Detect Apple Silicon hardware, research Metal/MLX/Core ML options, and implement custom local-LLM speed patches with measured quality-preserving gains. Use for Apple Silicon agents, GPU/Neural Engine partitioning, unified RAM, SSD caching or speculative decoding."
---

# Apple Silicon LLM Speed Lab
...
This edition targets Apple Silicon, with the actual SoC/RAM/runtime detected—not
fixed settings from the example deployment. Read
[platform-tuning.md](references/platform-tuning.md) after hardware detection.
The supplied system probes target macOS on Apple Silicon. Apple Silicon running
another OS needs an explicit driver/runtime adaptation; this is not validated
by merely detecting ARM64. For Windows/Linux PCs use `windows-linux-llm-speed-lab`.
Intel Macs are outside the Apple Silicon kernels, not outside investigation:
adapt the generic implementation workflow to their actual CPU/Metal runtime and
mark that port unvalidated until tested. Do not force MLX/ANE assumptions onto it.
